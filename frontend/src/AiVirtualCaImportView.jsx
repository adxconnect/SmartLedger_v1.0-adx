...
export default function AiVirtualCaImportView({ userUid, refreshTrigger, onTransactionsSaved }) {
    ...
    const [activeConversationId, setActiveConversationId] = useState(() => {
        return localStorage.getItem('ai_active_conv_id') || null;
    });
    ...
    useEffect(() => {
        ...
        if (activeConversationId) {
            localStorage.setItem('ai_active_conv_id', activeConversationId);
        } else {
    ...
    useEffect(() => {
        return () => {
            ...
            if (currentHistory && currentHistory.length > 1) {
                const firstUserMsg = currentHistory.find(m => m.sender === 'user');
                if (firstUserMsg) {
                    const dynamicTitle = firstUserMsg.text.substring(0, 30) + '...';
                    let updatedConversations = [...currentConversations];

                    if (currentId) {
                        const idx = updatedConversations.findIndex(c => String(c.id) === String(currentId));
                        if (idx !== -1) {
                            updatedConversations[idx] = { ...updatedConversations[idx], history: currentHistory };
                        } else {
                            updatedConversations.unshift({ id: currentId, history: currentHistory, title: dynamicTitle, date: new Date().toLocaleString() });
                        }
                    } else {
                        updatedConversations.unshift({ id: Date.now(), history: currentHistory, title: dynamicTitle, date: new Date().toLocaleString() });
                    }
                    
                    updatedConversations = updatedConversations.slice(0, 25);
                    localStorage.setItem('ai_conversations', JSON.stringify(updatedConversations));
                }
            }
            
            // Clear current chat state in localStorage so next mount starts fresh
    ...
    const portfolioContext = React.useMemo(() => {
        ...
        // 3. Live Investments
        const mfInvestments = invs.filter(i => i.assetType === 'Mutual Fund' || i.assetType === 'Stock');
        const goldInvestments = invs.filter(i => i.assetType && (i.assetType.toLowerCase().includes('gold') || i.assetType.toLowerCase().includes('metal')));
        const genInvestments = invs.filter(i => !mfInvestments.includes(i) && !goldInvestments.includes(i));

        const totalMfVal = mfInvestments.reduce((sum, m) => sum + (parseFloat(m.amountInvested) || 0), 0);
        const totalGoldVal = goldInvestments.reduce((sum, g) => sum + (parseFloat(g.amountInvested) || 0), 0);
        const totalGenInvVal = genInvestments.reduce((sum, i) => sum + (parseFloat(i.amountInvested) || 0), 0);
        ...
        return {
            ...
            liveInvestments: {
                ...
                mutualFundsCount: mfInvestments.length,
                mutualFundsTotal: totalMfVal,
                goldSilverCount: goldInvestments.length,
                goldSilverTotal: totalGoldVal,
                generalInvestmentsTotal: totalGenInvVal,
                items: [
                    ...mfInvestments.map(m => `MF/Stock: ${m.investmentName || 'Asset'} (₹${parseFloat(m.amountInvested || 0).toLocaleString('en-IN')})`),
                    ...goldInvestments.map(g => `Gold/Silver: ${g.investmentName || 'Metal'} (₹${parseFloat(g.amountInvested || 0).toLocaleString('en-IN')})`),
                    ...genInvestments.map(i => `Inv: ${i.investmentName || 'Investment'} (₹${parseFloat(i.amountInvested || 0).toLocaleString('en-IN')})`)
                ]
            },
            lenDen: {
                totalLent: totalLent,
                                        ...
                                        <button 
                                            ...
                                            onClick={(e) => { 
                                                ...
                                                if (String(activeConversationId) === String(conv.id)) {
                                                    ...
                                                    setChatHistory([{ sender: 'ai', text: "👋 Welcome to Ledger AI! How can I help you today?" }]);
                                                }
                                            }}
                        ...
                        <button 
                            ...
                            onClick={() => {
                                if (chatHistory.length > 1) {
                                    if (conversations.length >= 25 && !activeConversationId) {
                                    ...
                                    if (activeConversationId) {
                                        setConversations(prev => prev.map(c => String(c.id) === String(activeConversationId) ? { ...c, history: chatHistory } : c));
                                    } else {
                                        setConversations([{ 
                                            id: Date.now(), 
                                            history: chatHistory,
